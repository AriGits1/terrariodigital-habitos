...
/** Builds the contextual snapshot the Coach reasons over: done vs pending today, full habit list, and current hour. */
export async function getCoachContext(profileId: string): Promise<CoachContext> {
  ...
  return {
    ...
    habits: habits.map((h) => ({ id: h.id, title: h.title, weight: h.weight })),
